...
function doInsert(){
	...
	function doDelete(){
		...
		//	$_SESSION['ORDEREDNUM'] = $_POST['ORDEREDNUM'];
			...
			$summary->reference_number	= $_POST['reference'];
	...
	function addwishlist(){
		...
		function doInquire(){
			if(isset($_POST['submitinquiry'])){
				...
				$inquire = New Inquiry(); 
				$inquire->INQNAME 		= $_POST['name'];
				$inquire->INQEMAIL 		= $_POST['email']; 		
				$inquire->INQSUBJ  		= $_POST['subject']; 
				$inquire->INQMSG 		= $_POST['message'];
				$inquire->INQMSG 		= $_POST['message'];
				$inquire->INQDATECREATED 		= $date;
				$inquire->INQSTATUS 		= "NEW";
				$inquire->create();
				
				echo "<script> alert('Inquiry has been sent successfully.'); </script>";
