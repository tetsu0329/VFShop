...
function doInsert(){
	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
				...
				$mydb->setQuery($query);
				$cura = $mydb->loadResultList();
				foreach ($cura as $resulta) {


				$refnum = $resulta->reference_number;
				if($refnum=='NA')
				{
					$payment2 = $resulta->PAYMENT;
				}
				else
				{
				$payment1 = $resulta->PAYMENT;
				$payment2 = $payment1/2;
